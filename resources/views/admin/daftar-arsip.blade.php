    ...
    <div class="content">
      ...
      <main class="main-content">
        ...
        <div class="filter-and-action">
          <form method="GET" action="{{ route('admin.daftar-arsip') }}" class="filter-bar">
            @php
              use Carbon\Carbon;
            ...
            <a href="{{ route('admin.daftar-arsip') }}" class="btn-reset" title="Reset Filter">
              <i class="fas fa-arrows-rotate"></i>
            </a>
          </form>

          <div class="add-container">
            <a href="{{ route('arsip-konten.create') }}">
        ...
        <div class="export-buttons">
          <a href="{{ route('arsip.cetak.pdf', request()->all()) }}" class="btn-cetak" target="_blank">
          ...
          <tbody>
            @forelse ($arsips as $index => $arsip)
              <tr>
                ...
                <td>
                  @if ($arsip->gambar)
                    <img src="{{ asset('storage/' . $arsip->gambar) }}" alt="Gambar" style="width: 80px; height: auto;" />
                  @else
                    -
                  @endif
                </td>
                <td>
                  <div class="action-buttons">
                    <a href="{{ route('arsip-konten.edit', $arsip->id) }}" class="edit-btn" title="Edit"><i class="fas fa-edit"></i></a>
                    
                    <a href="{{ route('arsip.grafik', $arsip->id) }}" class="grafik-btn" title="Lihat Grafik">
                      <i class="fas fa-chart-pie"></i>
                    </a>
                    
                    <form action="{{ route('arsip-konten.destroy', $arsip->id) }}" method="POST" onsubmit="return confirm('Yakin hapus?')">
                      @csrf
                      @method('DELETE')
