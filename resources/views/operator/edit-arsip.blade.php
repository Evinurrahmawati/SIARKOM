...
<head>
  <meta charset="UTF-8">
  <title>Edit Arsip (Operator)</title>
...
<body>
  ...
  <div class="container">
    ...
    <form method="POST" action="{{ route('operator.arsip.update', $arsip->id) }}" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      ...
      <!-- Tema -->
      <div>
        <label for="id_tema">Pilih Topik</label>
      ...
      <div>
        ...
        <input type="text" name="judul" id="judul" value="{{ $arsip->judul }}" required>
      </div>

      <!-- Tanggal -->
      <div>
        <label for="tanggal">Tanggal</label>
        <input type="date" name="tanggal" id="tanggal" value="{{ $arsip->tanggal }}" required>
      </div>

      <!-- Jam -->
      <div>
        <label for="jam">Jam</label>
        <input type="text" name="jam" id="jam" value="{{ \Carbon\Carbon::createFromFormat('H:i:s', $arsip->jam)->format('H:i') }}" required>
      </div>

      <!-- Like -->
      <div>
        <label for="like">Jumlah Like</label>
        <input type="number" name="like" id="like" value="{{ $arsip->like }}" placeholder="Masukkan jumlah like">
      </div>

      <!-- View -->
      <div>
        <label for="view">Jumlah View</label>
        <input type="number" name="view" id="view" value="{{ $arsip->view }}" placeholder="Masukkan jumlah view">
      </div>
      ...
      <div>
        ...
        @if ($arsip->gambar)
          <p>Gambar saat ini:</p>
          <img src="{{ asset('storage/' . $arsip->gambar) }}" alt="Gambar" style="max-width: 100px;">
        @endif
      </div>
  ...
  <!-- AJAX: Ubah isi akun ketika platform berubah -->
  <script>
